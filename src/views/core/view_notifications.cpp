...
namespace big
{
	float draw_notification(float start_pos, ImDrawList* dl, const notification& notif)
	{
		...
		// Menggambar kotak notifikasi dengan sudut melengkung
		float rounding = 10.f; // Nilai rounding untuk sudut yang melengkung
		...
		// Menggambar latar belakang dengan warna gradient
		...
		auto title =
		    count > 1 ? std::format("{} ({})", notif.title(), count < 100 ? std::to_string(count) : "+99") : notif.title();
		...
		{
			dl->AddText(g.window.font_sub_title, 18.f, {(float)*g_pointers->m_gta.m_resolution_x - 350.f, 40.f + (i * 20.f) + start_pos}, textCol, txt.c_str());
			i++;
		}
